# =============================================================================
# Cap. 25.2 — CONVOLUCIÓN (3/3): IMPLEMENTACIÓN MANUAL DE LA CONVOLUCIÓN
# -----------------------------------------------------------------------------
# Propósito  : Implementar la convolución 1D "a mano" (doble for) para
#              entender qué hace internamente `np.convolve`. Esta es la
#              forma más básica de un FILTRO FIR cuando se programa en C
...
#                              y[n] += x[n-m] * h[m]
#
#              Comparar visualmente el resultado contra `np.convolve` para
#              validar la implementación.
# Complejidad: O(N*M) operaciones MAC (multiply-accumulate). Es el costo
#              típico de un FIR de M taps procesando N muestras.
...
# Equivalente a np.convolve(x, h, mode='full'). Sirve como REFERENCIA
# DIDÁCTICA y como esqueleto para traducir a C dentro de un µC.
def conv1d(x, h):
    N = len(x)        # longitud de la señal
    M = len(h)        # longitud del kernel (taps del FIR)
    L = N + M - 1     # longitud de la salida en modo 'full'
    y = np.zeros(L)
    for n in range(L):
        for m in range(M):
            if n - m >= 0 and n - m < N:
