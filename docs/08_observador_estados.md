...
$$
x[k+1] = \Phi\, x[k] + \Gamma\, u[k],\qquad
y[k] = C_d\, x[k]
$$

con $\Phi = e^{A T_s}$, $\Gamma = \int_0^{T_s} e^{A\tau}d\tau\, B$, y $C_d = [0\;0\;1]$ (solo medimos $\theta$).
...
$$
\mathcal{O} = \begin{bmatrix} C_d \\ C_d\,\Phi \\ C_d\,\Phi^2 \\ \vdots \\ C_d\,\Phi^{n-1} \end{bmatrix},\qquad \mathrm{rank}\,\mathcal{O} = n
$$
...
Idea: copiar el modelo de la planta **dentro del microcontrolador** y corregirlo con la **innovación** $y[k] - C_d\,\hat{x}[k]$ (diferencia entre lo medido y lo que el modelo predice).

$$
\boxed{\;\hat{x}[k+1] = \Phi\,\hat{x}[k] + \Gamma\,u[k] + L\,\bigl(y[k] - C_d\,\hat{x}[k]\bigr)\;}
$$
...
→ el error se rige **únicamente** por la dinámica $(\Phi - L\,C_d)$, **independiente** de $u[k]$ y $r[k]$. Si elegimos $L$ tal que los autovalores de $\Phi - L\,C_d$ estén estrictamente dentro del círculo unitario, $e[k]\to 0$ y $\hat{x}\to x$ asintóticamente.
...
$$
\begin{aligned}
x[k+1] - \hat{x}[k+1] &= \Phi\,x[k] + \Gamma\,u[k] \\
&\quad - \Phi\,\hat{x}[k] - \Gamma\,u[k] - L\bigl(y[k] - C_d\hat{x}[k]\bigr) \\
&= \Phi(x[k] - \hat{x}[k]) - L\,C_d\bigl(x[k] - \hat{x}[k]\bigr) \\
&= (\Phi - L C_d)\,e[k].
\end{aligned}
$$
...
El problema "ubicar polos de $\Phi - L\,C_d$" es **dual** a "ubicar polos de $\Phi - \Gamma K_z$":

$$
\mathrm{eig}(\Phi - L\,C_d) \;=\; \mathrm{eig}\bigl((\Phi - L\,C_d)^T\bigr) \;=\; \mathrm{eig}(\Phi^T - C_d^T\,L^T)
$$
...
Al cerrar el lazo $u[k] = -K_z\,\hat{x}[k] + K_{dc}\,r[k]$, el sistema completo en coordenadas $(x, e)$ queda:

$$
\begin{bmatrix} x[k+1] \\ e[k+1] \end{bmatrix} =
\underbrace{\begin{bmatrix} \Phi - \Gamma K_z & \Gamma K_z \\ 0 & \Phi - L\,C_d \end{bmatrix}}_{A_{\mathrm{aug}}}
\begin{bmatrix} x[k] \\ e[k] \end{bmatrix} +
\begin{bmatrix} \Gamma K_{dc} \\ 0 \end{bmatrix} r[k]
$$

La matriz es **triangular por bloques**, por lo que sus autovalores son la **unión** de los autovalores de $(\Phi - \Gamma K_z)$ y $(\Phi - L\,C_d)$. Conclusión:
...
4. Simula **dos** sistemas en paralelo:
   - **Ideal:** $u = -K_z\,x$ (acceso al estado real, no realizable).
   - **Realista:** $u = -K_z\,\hat{x}$ con el observador corriendo en lazo.
...
$$ s^{\text{obs}}_{1,2} = -11{,}51 \pm j\,15{,}68,\qquad s^{\text{obs}}_3 = -115{,}1 $$

$$ z^{\text{obs}}_{1,2} = e^{-0{,}1151}\bigl(\cos 0{,}1568 \pm j\sin 0{,}1568\bigr) \approx 0{,}881 \pm j\,0{,}139 $$
